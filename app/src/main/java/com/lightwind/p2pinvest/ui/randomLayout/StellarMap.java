package com.lightwind.p2pinvest.ui.randomLayout;
...
public class StellarMap extends FrameLayout implements AnimationListener, OnTouchListener, OnGestureListener {
    ...
    private void init() {
        ...
        mHideGroupIndex = -1;
        mShownGroupIndex = -1;
        mHideGroup = new RandomLayout(getContext());
        mShownGroup = new RandomLayout(getContext());

        addView(mHideGroup, new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
        mHideGroup.setVisibility(View.GONE);
    ...
    public void setRegularity(int xRegularity, int yRegularity) {
        mHideGroup.setRegularity(xRegularity, yRegularity);
    ...
    private void setChildAdapter() {
        ...
        mHideGroupAdapter = new RandomLayout.Adapter() {
            //取出本Adapter的View对象给HideGroup的Adapter
            @Override
            public View getView(int position, View convertView) {
                return mAdapter.getView(mHideGroupIndex, position, convertView);
            }

            @Override
            public int getCount() {
                return mAdapter.getCount(mHideGroupIndex);
            }
        };
        mHideGroup.setAdapter(mHideGroupAdapter);
    ...
    public void setInnerPadding(int paddingLeft, int paddingTop, int paddingRight, int paddingBottom) {
        mHideGroup.setPadding(paddingLeft, paddingTop, paddingRight, paddingBottom);
    ...
    private void switchGroup(int newGroupIndex, boolean playAnimation, Animation inAnim, Animation outAnim) {
        ...
        if (playAnimation) {
            ...
            mHideGroup.startAnimation(outAnim);
        } else {
            mHideGroup.setVisibility(View.GONE);
        }
    }
